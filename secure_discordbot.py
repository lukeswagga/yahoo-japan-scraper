...
@bot.command(name='test_new_user_bookmark')
@commands.has_permissions(administrator=True)
async def test_new_user_bookmark(ctx, test_username=None):
    """Safely test bookmark channel creation for a new user without affecting existing channels"""
    ...
    try:
        ...
        # Create test bookmark data (similar to a real listing)
        test_auction_data = {
            'auction_id': f'test{random.randint(100000, 999999)}',
            'title': 'TEST LISTING - Raf Simons Archive Redux Sample',
            'brand': 'raf_simons',
            'price_jpy': 25000,
            'price_usd': 170.50,
        ...
        if not bookmarks_category:
            await ctx.send("❌ USER BOOKMARKS category not found")
            return
        
        # Create test channel name
        channel_name = f"bookmarks-{test_username.lower()}"
        
        # Check if test channel already exists
        existing_test_channel = discord.utils.get(guild.text_channels, name=channel_name)
        if existing_test_channel:
            await ctx.send(f"🧹 Test channel already exists, deleting it first...")
            await existing_test_channel.delete()
        
        # Create the test bookmark channel
        overwrites = {
            guild.default_role: discord.PermissionOverwrite(read_messages=False),
            guild.me: discord.PermissionOverwrite(read_messages=True, send_messages=True),
            ctx.author: discord.PermissionOverwrite(read_messages=True, send_messages=True)  # Give you access to see it
        }
        ...
        bookmark_channel = await guild.create_text_channel(
            ...
            overwrites=overwrites,
            topic=f"Private bookmark channel for {test_username} (TEST CHANNEL)"
        )
        
        # Send welcome message
        welcome_embed = discord.Embed(
            title="📚 TEST Bookmark Channel Created!",
            description=f"This is a test bookmark channel for user: **{test_username}**\n\nThis demonstrates the new user bookmark channel creation process.",
            color=0x0099ff
        )
        welcome_embed.add_field(
            name="🧪 Test Status:",
            value="• Channel created successfully\n• Proper permissions set\n• Category placement correct\n• Ready for bookmark testing",
            inline=False
        )
        
        await bookmark_channel.send(embed=welcome_embed)
        
        # Test sending a bookmark to the channel
        bookmark_embed = discord.Embed(
            title=test_auction_data['title'],
            url=test_auction_data['zenmarket_url'],
            description=f"**Brand:** {test_auction_data['brand'].replace('_', ' ').title()}\n**Price:** ¥{test_auction_data['price_jpy']:,} (${test_auction_data['price_usd']:.2f})\n**Deal Quality:** {test_auction_data['deal_quality']}/100",
            color=0x00ff00,
            timestamp=datetime.now(timezone.utc)
        )
        bookmark_embed.set_footer(text=f"TEST BOOKMARK | ID: {test_auction_data['auction_id']}")
        
        await bookmark_channel.send(embed=bookmark_embed)
        
        # Send success message to command channel
        success_embed = discord.Embed(
            title="✅ New User Bookmark Test Successful!",
            description=f"Test completed for user: **{test_username}**",
            color=0x00ff00
        )
        success_embed.add_field(
            name="✅ Test Results:",
            value=f"• Channel created: #{channel_name}\n• Category: 📚 USER BOOKMARKS\n• Permissions: Private to test user\n• Welcome message: Sent\n• Test bookmark: Posted",
            inline=False
        )
        success_embed.add_field(
            name="🔧 Next Steps:",
            value="Check the new channel to see the new user experience. Delete test channel when done testing.",
            inline=False
        )
        
        await ctx.send(embed=success_embed)
        print(f"✅ Test bookmark channel created: #{channel_name}")
        
    except Exception as e:
        await ctx.send(f"❌ Test failed: {str(e)}")
        print(f"❌ Error in bookmark test: {e}")
        import traceback
        traceback.print_exc()

@bot.command(name='cleanup_test_bookmarks')
@commands.has_permissions(administrator=True)
async def cleanup_test_bookmarks(ctx):
    """Clean up any test bookmark channels"""
    
    guild = ctx.guild
    test_channels = [ch for ch in guild.text_channels if ch.name.startswith('bookmarks-testuser')]
    
    if not test_channels:
        await ctx.send("✅ No test bookmark channels found to clean up")
        return
    ...
    for channel in test_channels:
        try:
            ...
            deleted_count += 1
            print(f"🧹 Deleted test channel: #{channel.name}")
        except Exception as e:
            print(f"❌ Could not delete {channel.name}: {e}")
    
    await ctx.send(f"🧹 Cleaned up {deleted_count} test bookmark channels")
